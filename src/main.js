...
bot.command("new", async (ctx) => {
  ...
  await ctx.reply("Waiting your voice message");
});
...
bot.on(message("voice"), async (ctx) => {
  ...
  try {
    await ctx.reply(code("Request recieved, waiting respoce from server"));
...
bot.on(message("text"), async (ctx) => {
  ctx.session = ctx.session ?? INITIAL_SESSION;
  try {
    await ctx.reply(code("Request recieved, waiting response from server"));
    ctx.session.messages.push([
